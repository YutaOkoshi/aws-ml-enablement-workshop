...
生成 AI の出力が期待通りにならないときは、次の 3 つで立て直してください。「必ず〜してください」のような強い言葉をプロンプトの語尾に足す対応は、生成 AI がその一点に過剰に反応して他の指示を崩すことがあるため避けてください。

* **出力が曖昧なとき** : 参加者が知っている事実を前提知識として足します。「私達のサービスのユーザーは○○です」のように、生成 AI が知り得ない情報を渡すと具体性が上がります
* **出力形式が崩れるとき** : 望ましい形を 1 例だけ見せます。表や箇条書きの 1 行を書いて渡すのが手軽で、実際に欲しいものに近く互いに異なる例ほど効果があります
* **深掘りが足りないとき** : 使っているツールの thinking / reasoning effort を上げます
...
* 対応
   ...
   * 時系列に並ばない、行動の粒度が荒いときは、望ましい 1 行を書いて見せる (「10:00 に○○の画面を開き、△分かけて□□を入力した」など)
   * 顧客の行動が一般論にとどまるときは、参加者が知っている事実を足す (「私達のサービスのユーザーは○○です」「○○する人はほとんどいません」など)
